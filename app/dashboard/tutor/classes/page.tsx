  ...
  return <LmsShell profile={profile} activeRole="tutor" activePath="/dashboard/tutor/classes" eyebrow="Live teaching" title="My classes" subtitle={`Every class is shown in ${timezone}. A session counts as delivered only after the student confirms it.`} actions={<a className="module-link" href="/dashboard/tutor/classes/new">+ Schedule class</a>}>
    ...
    <div className="availability-layout">
      ...
      <aside className="panel availability-windows-panel">
        ...
        <details className="extra-slot-editor"><summary>+ Add another window</summary><form action="/api/lms" method="post"><input type="hidden" name="action" value="save-availability-slot"/><input type="hidden" name="tutorId" value={tutorId}/><label>Day<select name="weekday">{days.map((day,index)=><option value={index} key={day}>{day}</option>)}</select></label><div className="form-row"><TimePicker12 name="startTime" label="From" required/><TimePicker12 name="endTime" label="Until" defaultValue="17:00" required/></div><label>Timezone<select name="timeZone" defaultValue={timezone}>{supportedTimeZones().map(zone=><option value={zone} key={zone}>{zone.replaceAll("_"," ")}</option>)}</select></label><button>Open extra slot</button></form></details>
      </aside>
    </div>
