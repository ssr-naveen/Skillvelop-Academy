...
export default async function Page({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  const users = await all<Row>(`SELECT u.id,u.name,u.email,u.role,u.status,GROUP_CONCAT(mp.permission, ', ') permissions FROM users u LEFT JOIN manager_permissions mp ON mp.manager_id=u.id WHERE u.status='active' GROUP BY u.id ORDER BY CASE u.role WHEN 'admin' THEN 0 WHEN 'manager' THEN 1 ELSE 2 END,u.created_at DESC`);
