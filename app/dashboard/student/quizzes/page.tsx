...
import QuizQuestionResource from "./QuizQuestionResource";

type Quiz={id:string;title:string;description:string;question_count:number;attempts:number;best:number|null};
type Question={id:string;quiz_id:string;type:string;prompt:string;options_json:string;answer_json:string;points:number;position:number;resource_type:string;resource_embed_url:string;image_file_name:string;image_content_type:string;image_size_bytes:number};
...
export default async function Page({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const [quizzes,questions,attempts]=await Promise.all([
    ...
    all<Question>(`SELECT qq.id,qq.quiz_id,qq.type,qq.prompt,qq.options_json,qq.answer_json,qq.points,qq.position,qq.resource_type,qq.resource_embed_url,qq.image_file_name,qq.image_content_type,qq.image_size_bytes
