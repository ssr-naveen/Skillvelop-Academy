...
const roleNav: Record<WorkspaceRole, { href: string; label: string; icon: LucideIcon; permission?: ManagerPermission }[]> = {
  admin: [
    ...
    { href: "/dashboard/admin/students", label: "Student plans", icon: GraduationCap, permission: "manage_students" }, { href: "/dashboard/admin/curriculum", label: "Curriculum library", icon: LibraryBig, permission: "manage_courses" },
...
export function Notice({ params }: { params?: { created?: string; updated?: string; submitted?: string; sent?: string; deleted?: string; error?: string; score?: string; max?: string } }) {
  ...
  const success = params.created === "user" ? "User created with a permanent Skillvelop ID." : params.created === "manager" ? "Manager created with customized permissions." : params.created === "course" ? "Course created in locked mode. Add chapters before releasing it." : params.created === "chapter" ? "Chapter added to the course structure." : params.created === "quiz" ? "Knowledge check created. Add its interactive questions." : params.created === "class" ? "Live class scheduled in the selected timezone." : params.created === "activity" ? "Learning activity published." : params.created === "lesson" ? "Curriculum lesson published." : params.created === "template" ? "Curriculum package added to the library." : params.updated === "credential" ? "The VIYU API credential was encrypted and connected." : params.updated === "ai" ? "VIYU configuration saved." : params.updated === "feedback" ? "Feedback returned to the learner." : params.updated === "subscription" ? "Student plan and class entitlement updated." : params.updated === "renewal" ? "Subscription renewed successfully." : params.updated === "lesson" ? "Lesson completed — 50 XP added." : params.updated === "profile" ? "Profile and public tutor page updated." : params.updated === "template-lesson" ? "Lesson added to the curriculum package." : params.updated === "enrolment" ? "Student enrolled in the course." : params.updated === "resource" ? "Learning resource uploaded to the lesson." : params.updated === "question" ? "Interactive question added." : params.updated === "unlock" ? "Student access setting updated." : params.updated === "confirmation-requested" ? "Confirmation request sent to the student." : params.updated === "class-confirmed" ? "Class confirmed and tutor rating recorded." : params.deleted === "course" ? "Course and its learning records were deleted." : params.deleted === "curriculum" ? "Curriculum package was deleted. Existing imported courses were preserved." : params.sent ? "Message sent." : params.submitted === "quiz" ? `Quiz submitted. Score: ${params.score ?? "0"}/${params.max ?? "0"}.` : params.submitted ? "Your work was submitted to your tutor." : "";
  const error = params.error ? "Please check the required fields and try again." : "";
