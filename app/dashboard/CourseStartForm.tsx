...
export default function CourseStartForm({mode,curricula,subjects,tutors=[],students=[]}:{mode:"admin"|"tutor";curricula:Curriculum[];subjects:Subject[];tutors?:Person[];students?:Person[]}){
  const [curriculumId,setCurriculumId]=useState("");const [subjectId,setSubjectId]=useState("");const [grade,setGrade]=useState("");const [courseMode,setCourseMode]=useState<"guided"|"self_paced">("guided");
  ...
  return <form action="/api/lms" method="post" className="guided-course-form">
    ...
    <header><span><Layers3 size={20}/></span><div><small>COURSE SETUP</small><h2>Start with the learning structure</h2><p>Select the delivery model, curriculum, grade and subject. Chapters and classroom materials come next.</p></div></header>
    <fieldset className="course-mode-choice"><legend>Delivery model</legend><label className={courseMode==="guided"?"selected":""}><input type="radio" name="courseMode" value="guided" checked={courseMode==="guided"} onChange={()=>setCourseMode("guided")}/><span><strong>Guided classroom</strong><small>Tutor-controlled unlocks and optional chapter drip dates.</small></span></label><label className={courseMode==="self_paced"?"selected":""}><input type="radio" name="courseMode" value="self_paced" checked={courseMode==="self_paced"} onChange={()=>setCourseMode("self_paced")}/><span><strong>Self-paced pathway</strong><small>Always available after assignment; each completed step reveals the next.</small></span></label></fieldset>
    ...
    <button disabled={!ready}><BookOpenCheck size={17}/> Create {courseMode==="self_paced"?"self-paced":"guided"} course</button>
