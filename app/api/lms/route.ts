...
import { loadLearningSequence } from "@/db/learning-sequence";
...
async function sequenceAllows(courseId:string,studentId:string,type:"lesson"|"quiz"|"activity",id:string) {
  const sequence=await loadLearningSequence(courseId,studentId);
  return Boolean(sequence.find(item=>item.type===type&&item.id===id)?.available);
}
...
export async function POST(request: Request) {
  ...
  if (action === "complete-lesson") {
    ...
    if(!(await sequenceAllows(assigned.course_id,studentId,"lesson",lessonId)))return NextResponse.json({error:"Complete the previous learning item first"},{status:409});
  ...
  if (action === "submit-activity") {
    ...
    if(!(await sequenceAllows(assigned.course_id,studentId,"activity",activityId)))return NextResponse.json({error:"Complete the previous learning item first"},{status:409});
