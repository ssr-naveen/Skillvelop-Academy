...
export async function GET(_:Request,{params}:{params:Promise<{publicId:string}>}){
  const {publicId}=await params;const row=await database().prepare("SELECT p.avatar_r2_key,p.avatar_content_type FROM users u JOIN user_profiles p ON p.user_id=u.id WHERE u.public_id=? AND u.status='active' AND p.is_public=1 AND p.avatar_r2_key!=''").bind(publicId).first<{avatar_r2_key:string;avatar_content_type:string}>();
