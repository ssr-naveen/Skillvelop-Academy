...
const presentation=`Write responses in clean, polished Markdown that is easy to scan on mobile. Use short paragraphs, descriptive headings when useful, bullet points for lists, and numbered steps for processes. Use **bold** for important terms and *italics* sparingly for emphasis. Emojis are welcome when they add warmth or clarity, but keep them tasteful and professional rather than excessive. Do not show raw Markdown syntax unnecessarily. Avoid giant walls of text, excessive headings, repetitive introductions, or robotic phrasing. Keep the tone warm, upbeat, confident, jovial and professional. Format all mathematics in LaTeX wrapped in $ delimiters.`;
...
export async function POST(request:Request){
  ...
  const personalization=userName?`The current user's account name is ${JSON.stringify(userName)}. Address them naturally by their name when greeting them, acknowledging progress, encouraging them, or when it improves warmth and clarity. Prefer their first name in conversational replies when appropriate. Do not repeat their name in every paragraph or sentence, and never invent a different name.`:"";
