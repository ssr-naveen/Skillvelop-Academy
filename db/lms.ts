...
const schema = [
  ...
  `CREATE TABLE IF NOT EXISTS quizzes (id TEXT PRIMARY KEY, course_id TEXT NOT NULL REFERENCES courses(id), chapter_id TEXT REFERENCES chapters(id), tutor_id TEXT NOT NULL REFERENCES users(id), title TEXT NOT NULL, description TEXT NOT NULL, kind TEXT NOT NULL DEFAULT 'quiz', attempt_limit INTEGER NOT NULL DEFAULT 1, is_unlocked INTEGER NOT NULL DEFAULT 0, status TEXT NOT NULL DEFAULT 'published', created_at TEXT NOT NULL)`,
