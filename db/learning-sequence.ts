...
type QuizRow = { id:string;course_id:string;chapter_id:string|null;chapter_title:string|null;chapter_position:number|null;chapter_unlocked:number|null;title:string;description:string;kind:string;is_unlocked:number;attempts:number };
...
export async function loadLearningSequence(courseId: string, studentId: string) {
  const db = database();
  const [lessonResult, quizResult, activityResult] = await Promise.all([
  ...
  return items.map(item => ({ ...item, available: item.tutorUnlocked }));
}
